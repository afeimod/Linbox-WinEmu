 ...
 * 弹窗: 安装图形桌面.
 *
 * 进入弹窗**不会自动安装**, 由用户选 DE 后点"开始安装"按钮触发.
 * 父 Composable 持有 [vm] (用 [rememberInstallDesktopController]), 状态稳定不丢.
 *
 * 用法:
...
fun InstallDesktopDialog(
    ...
    // 进入弹窗: 仅当 rootfs 切换 (state.rootfsPath 还没记录) 时才 init
    // 不会自动 start(), 用户选完 DE 点"开始安装"才跑
