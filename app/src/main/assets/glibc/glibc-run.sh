...
# glibc-run.sh
#
# glibc-run 是被 FIFO server 派发时执行的命令, 准备 box64+wine 环境, 然后 exec。
#
# 注意: 这个脚本跑在 proot 内的 imagefs 镜像下, imagefs bind 到 /imagefs
#   - /imagefs 是 imagefs bind mount
#   - /imagefs/tmp 是 glibc rootfs 的 tmp (放 .sysvshm 等, 跟 FIFO 无关)
#   - /imagefs/usr/bin/box64: musl 链接的 box64
#   - /imagefs/opt/wine/bin/wine: glibc 链接的 wine
#   - /imagefs/usr/lib/ld-linux-aarch64.so.1: glibc loader
#
# 调用方式 (proot 内, 由 FIFO server 派发):
#   glibc-run winecfg
#   glibc-run /home/xuser/.wine/drive_c/foo.exe
#   glibc-run wine /path/foo.exe
#   glibc-run box64 <args>     # 完全透传
#   glibc-run ls /imagefs      # 其他 binary
...
# ============================================================
# 3) TMPDIR / DISPLAY
# 注意: TMPDIR 用 Android 视角的绝对路径 (跟 FIFO 路径一致)
# ============================================================
